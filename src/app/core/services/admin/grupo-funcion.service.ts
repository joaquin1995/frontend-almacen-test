...
import { GrupoFuncion } from '../../models';

@Injectable({
  providedIn: 'root'
})
export class GrupoFuncionService {
  parametros: string;
  urlPath = '/api/grupofuncion';
...
// tslint:disable-next-line: max-line-length
  ...
  guardar(modelo: GrupoFuncion): Observable<any> {
    return this.apiService.post(this.urlPath,  modelo )
      .pipe(map(data => data));
  }

  modificar(codigo: string, modelo: GrupoFuncion): Observable<any> {
